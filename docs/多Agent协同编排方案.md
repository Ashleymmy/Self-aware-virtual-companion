...
**验收标准：**
- [ ] `savc-core/agents/` 下有 6 个 YAML 定义文件
- [ ] `node savc-core/orchestrator/registry.mjs list` 输出所有已注册 Agent
- [ ] `node savc-core/orchestrator/registry.mjs match --intent "emotional_support"` 返回 companion
- [ ] `node savc-core/orchestrator/registry.mjs match --text "帮我看看这个 bug"` 返回 technical
- [ ] 新增一个 YAML 文件后，无需重启即可被发现（热重载）
...
**验收标准：**
- [ ] "抱抱我" → companion（Level 1 关键词命中）
- [ ] "帮我优化这段 SQL 查询" → technical（Level 2 LLM 分类）
- [ ] "你觉得人生的意义是什么" → 回退到 orchestrator（Level 3）
- [ ] 路由决策耗时：Level 1 < 5ms，Level 2 < 500ms
- [ ] 路由日志记录：消息摘要、匹配级别、目标 Agent、耗时
...
**验收标准：**
- [ ] "帮我写个爬虫，顺便记住我喜欢 requests" → 2 个并行子任务
- [ ] "先查一下昨天聊了什么，然后帮我继续那个项目" → 2 个串行子任务
- [ ] "今天心情不好" → simple 类型，不触发分解
- [ ] 分解结果包含完整的执行计划（agent、task、priority、dependsOn）
...
**验收标准：**
- [ ] `spawnAgent('companion', '抱抱用户')` 成功创建子 Agent 并返回 runId
- [ ] `waitForAgent(runId)` 正确等待并返回 Agent 输出
- [ ] 超时场景：Agent 超过 timeout 后被自动取消
- [ ] 错误场景：Agent 异常时错误被捕获并记录
- [ ] `waitForAll([id1, id2])` 正确等待多个并行 Agent
...
**验收标准：**
- [ ] 单 Agent 结果直接透传，无额外 LLM 调用
- [ ] 多 Agent 结果整合后语气统一，信息完整
- [ ] 部分失败时用户能知道哪部分没完成
- [ ] 聚合后的回复不暴露内部 Agent 架构
...
- [ ] 8 个集成测试场景全部通过
- [ ] 性能指标达到基线要求
- [ ] 现有 Phase 1-3 测试不受影响（`npm run test:all`）
...
| 新建 | `openclaw/extensions/savc-orchestrator/` | OpenClaw 编排扩展 |
| 修改 | `savc-core/SOUL.md` | 追加 Agent 协同指令 |
| 修改 | `config/models.yaml` | 新增各 Agent 模型配置 |
