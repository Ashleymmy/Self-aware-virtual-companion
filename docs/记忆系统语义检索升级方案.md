...
> 版本: 1.0
> 日期: 2026-02-09
> 状态: Phase 4a 已完成并完成收尾增强（2026-02-10）
...
结果：
- `scripts/test_phase4a.sh`：PASS（25/25）
- 包含 `memory_runtime` 三模式、双写、`load --query`、`auto-recall`、`auto-capture`、时间衰减字段、`migrate` 幂等、`health`、`usage.log` 与 Phase1/3 回归验证

---

### Step 8: 收尾增强记录（2026-02-10）

已执行命令（节选）：

```bash
node tests/skills/memory-semantic-local.test.mjs
node scripts/memory_runtime.mjs auto-recall --workspace /tmp/savc_phase4a_runtime/savc-core --query "Python" --json
node scripts/memory_runtime.mjs auto-capture --workspace /tmp/savc_phase4a_runtime/savc-core --text "请记住：我偏好 TypeScript" --json
bash scripts/test_phase4a.sh
```

本轮新增能力：
- [x] **本地 Embedding 模式**：`SAVC_EMBEDDING_MODE=local`，支持本地 endpoint（如 Ollama）
- [x] **时间衰减评分**：`score = rawScore × decay(age)`，并输出 `rawScore/decay/ageDays`
- [x] **Auto-Recall**：新增 `memory_semantic auto-recall` / `memory_runtime auto-recall`
- [x] **Auto-Capture**：新增 `memory_semantic auto-capture` / `memory_runtime auto-capture`
...
1. [x] **本地 Embedding 模型** — 已支持 `local` 模式（默认 endpoint: `http://127.0.0.1:11434/api/embeddings`）
2. [x] **记忆重要性衰减** — 已支持时间衰减重排：`final_score = similarity × decay(age)`
3. [ ] **记忆聚类与整理** — 定期对向量空间做聚类，自动发现主题并生成摘要
4. [x] **Auto-Capture 集成** — 已提供自动提取与语义写入命令
5. [x] **Auto-Recall 集成** — 已提供自动召回上下文命令
...
| 修改 | `scripts/memory_runtime.mjs` | 新增 `auto-recall` / `auto-capture` 命令 |
...
| 修改 | `scripts/memory_semantic.mjs` | 新增 local embedding、时间衰减、auto-recall、auto-capture |
| 运行时生成 | `savc-core/memory/vector/` | LanceDB 数据目录与 usage.log |
| 新建 | `tests/skills/memory-semantic.test.mjs` | 语义搜索测试 |
| 新建 | `tests/skills/memory-semantic-local.test.mjs` | 本地 embedding 模式测试 |
