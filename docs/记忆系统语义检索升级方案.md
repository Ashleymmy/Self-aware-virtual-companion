...
**创建向量存储目录：**
```bash
mkdir -p savc-core/memory/vector
echo "vector/" >> savc-core/memory/.gitignore  # 确保不被追踪
```

**验收标准：**
- [ ] `pnpm ls @lancedb/lancedb` 显示已安装
- [ ] `node -e "import('@lancedb/lancedb').then(m => console.log('ok'))"` 无报错
- [ ] `.env.local` 中 `OPENAI_API_KEY` 已配置
...
**验收标准：**
- [ ] `node scripts/memory_semantic.mjs store "用户喜欢用 Python 写脚本"` 成功写入
- [ ] `node scripts/memory_semantic.mjs search "编程语言偏好"` 能召回上条记忆
- [ ] `node scripts/memory_semantic.mjs search "Python"` 同样能召回（语义匹配）
- [ ] 重复存储同一文本不会产生新记录
...
**验收标准：**
- [ ] `search` 无 `--mode` 参数时行为与之前完全一致
- [ ] `write` 后 LanceDB 中能查到对应记忆
- [ ] `load --query "之前聊的技术话题"` 能召回相关 episodic 记忆
...
**验收标准：**
- [ ] 迁移完成后 `memory_semantic.mjs stats` 显示合理的记忆条数
- [ ] 对已有记忆内容的语义搜索能正确召回
- [ ] 迁移脚本可重复执行（幂等），不会产生重复记录
...
**验收标准：**
- [ ] proactive daemon 的话题跟进使用语义搜索
- [ ] self-reflection 能引用历史相关记忆
- [ ] 所有现有测试仍然通过（`npm run test:all`）
...
| 修改 | `config/.env.local` | 新增 `OPENAI_API_KEY`、`EMBEDDING_MODEL` |
| 新建 | `savc-core/memory/vector/` | LanceDB 数据目录（gitignore） |
