...
OpenClaw plugin workspace located at `packages/plugin`. It exposes these SAVC orchestration tools:
...
All config lives under `plugins.entries.savc-orchestrator.config`, and the plugin itself should be loaded from `packages/plugin`:
...
- `logFile?: string` (default `packages/core/memory/procedural/orchestrator.log`)

## Notes

- Default mode is `mock` for safe rollout.
- Default OpenClaw root is `<REPO_ROOT>/openclaw`; override with `OPENCLAW_ROOT` when needed.
