...
import { loadLifecycleModule, loadLive2DModule, loadMemorySemanticModule, loadRegistryModule, resolveRuntimeContext, } from "./paths.js";
import { readLatestAssistantReply, sendRealAgentMessage, spawnRealAgent, waitForRealAgentRun, } from "./real-session-adapter.js";
import { ensureRunRecord, patchRunRecord, toAgentRunResult as toStoredAgentRunResult, } from "./run-store.js";
...
function toAgentRunResult(snapshot) {
    return {
        ...
        output: snapshot.output === undefined || snapshot.output === null ? null : String(snapshot.output),
        durationMs: typeof snapshot.durationMs === "number" && Number.isFinite(snapshot.durationMs)
            ? snapshot.durationMs
            : null,
        error: snapshot.error === undefined || snapshot.error === null ? null : String(snapshot.error),
    };
}
async function appendPluginLog(ctx, line) {
    await fs.mkdir(path.dirname(ctx.logFilePath), { recursive: true });
    const timestamp = new Date().toISOString();
    await fs.appendFile(ctx.logFilePath, `[${timestamp}] ${line}\n`, "utf8");
}
async function tryRecall(memory, ctx, query, limit) {
...
function buildTaskWithMemory(task, recall) {
...
function inferLive2DSourceFromAgent(agentName) {
    if (agentName === "voice") {
        return "voice";
    }
    if (agentName === "live2d") {
...
function resolveRealSpawnContext(toolCtx) {
    ...
    const channel = typeof toolCtx.messageChannel === "string" && toolCtx.messageChannel.trim()
        ? toolCtx.messageChannel.trim()
        : undefined;
    const accountId = typeof toolCtx.agentAccountId === "string" && toolCtx.agentAccountId.trim()
...
export function createSpawnExpertTool(api, toolCtx) {
    return {
        name: "savc_spawn_expert",
        description: "Spawn SAVC expert agent via mock lifecycle (default) or real OpenClaw sessions backend.",
        parameters: Type.Object({
            ...
            persistMemory: Type.Optional(Type.Boolean({ description: "When agent=memory, persist task into semantic memory." })),
            useSessionsSend: Type.Optional(Type.Boolean({
                description: "Real mode only. Also send a coordination message via sessions_send to the child session.",
            })),
            handoffMessage: Type.Optional(Type.String({
                description: "Optional custom message when useSessionsSend=true.",
            })),
            handoffTimeoutSeconds: Type.Optional(Type.Number({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const recallQuery = typeof params.recallQuery === "string" && params.recallQuery.trim()
                ...
                const handoffMessage = typeof params.handoffMessage === "string" && params.handoffMessage.trim()
                    ? params.handoffMessage.trim()
                    : "";
                const handoffTimeoutSeconds = toPositiveNumber(params.handoffTimeoutSeconds, Math.max(1, Math.ceil(timeoutMs / 1000)));
                ...
                if (ctx.config.spawnMode === "real") {
                    ...
                    if (!realContext) {
                        const details = failure("MISSING_SESSION_CONTEXT", "real spawn mode requires tool context sessionKey and agentId");
                        return {
                            content: [
                                {
                                    type: "text",
                    ...
                    if (useSessionsSend) {
                        ...
                        const coordinationMessage = handoffMessage ||
                            [
                                "[协调消息]",
                                `runId=${realSpawn.runId}`,
                ...
                if (memory &&
                    ...
                    try {
                        const captureResult = await memory.autoCapture([task, finalTask, runResult.output || ""], {
                            workspace: ctx.savcCorePath,
                ...
                if (runResult.status === "completed") {
                    ...
                    try {
                        ...
                        if (typeof live2dModule.buildLive2DPlan === "function") {
                            ...
                            const planSignal = plan && typeof plan === "object" && plan.signal && typeof plan.signal === "object"
                                ? plan.signal
                                : null;
                            ...
                            live2d.interactionType =
                                toOptionalString(plan?.interactionType) ??
                                    toOptionalString(planSignal?.interaction?.type);
                        }
                        else {
                            ...
                            const signalData = signal && typeof signal === "object" ? signal : null;
                            ...
                            live2d.interactionType = toOptionalString(signalData?.interaction?.type);
                        }
                    }
                    catch (error) {
                ...
                await appendPluginLog(ctx, `spawn agent=${agentName} runId=${runResult.runId} status=${runResult.status} mode=${ctx.config.spawnMode} recallCount=${recallCount} persisted=${persisted} autoCapture=${autoCapture.stored} live2d=${live2d.attempted ? "on" : "off"} sessionsSend=${sessionsSend.attempted ? sessionsSend.status : "skipped"}`);
