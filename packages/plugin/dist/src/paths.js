...
function toAbsolutePath(api, input) {
    if (path.isAbsolute(input)) {
        return path.resolve(input);
    }
    return path.resolve(api.resolvePath(input));
}
function findRepoRoot(seedPaths) {
    for (const seed of seedPaths) {
        let cursor = path.resolve(seed);
        while (true) {
            // 新布局：scripts/runtime/memory_semantic.mjs
            const candidateNew = path.join(cursor, "scripts", "runtime", "memory_semantic.mjs");
            if (hasFile(candidateNew)) {
                return cursor;
            }
            // 兼容旧布局：scripts/memory_semantic.mjs
            const candidateOld = path.join(cursor, "scripts", "memory_semantic.mjs");
            if (hasFile(candidateOld)) {
                return cursor;
            }
...
function resolveSavcCorePath(api, pluginSavcCorePath) {
    if (pluginSavcCorePath) {
        return toAbsolutePath(api, pluginSavcCorePath);
    }
    const workspaceFromConfig = typeof api.config?.agents?.defaults?.workspace === "string"
...
function resolveLogFilePath(savcCorePath, configuredLogFile) {
...
export function resolveRuntimeContext(api, options = {}) {
    ...
    const repoRoot = findRepoRoot([
        ...
        process.cwd(),
        // packages/plugin/src/ → 向上 3 层到项目根
        path.resolve(THIS_DIR, "../../.."),
        path.resolve(THIS_DIR, "../.."),
    ]) ?? path.resolve(THIS_DIR, "../../..");
    // 新布局优先：scripts/runtime/memory_semantic.mjs
    const memorySemanticNewPath = path.join(repoRoot, "scripts", "runtime", "memory_semantic.mjs");
    const memorySemanticOldPath = path.join(repoRoot, "scripts", "memory_semantic.mjs");
    const memorySemanticPath = hasFile(memorySemanticNewPath) ? memorySemanticNewPath : memorySemanticOldPath;
...
export async function loadRouterModule(ctx) {
...
export async function loadDecomposerModule(ctx) {
...
export async function loadRegistryModule(ctx) {
...
export async function loadMemorySemanticModule(ctx) {
    ensurePathExists(ctx.memorySemanticPath, "memory semantic module");
    const mod = await importModule(ctx.memorySemanticPath);
...
export async function loadVisionModule(ctx) {
...
export async function loadLive2DModule(ctx) {
