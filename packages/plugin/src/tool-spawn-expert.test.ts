...
const mocked = vi.hoisted(() => {
  ...
  return {
    ...
    resolveRuntimeContext: vi.fn(() => ({
      ...
      savcCorePath: "/tmp/packages/core",
      agentsDir: "/tmp/packages/core/agents",
      orchestratorDir: "/tmp/packages/core/orchestrator",
      repoRoot: "/tmp/repo",
      memorySemanticPath: "/tmp/repo/scripts/runtime/memory_semantic.mjs",
...
describe("savc_spawn_expert tool", () => {
  ...
  it("spawns technical expert with semantic recall context", async () => {
    ...
    expect(mocked.discoverAgents).toHaveBeenCalledWith("/tmp/packages/core/agents", {
      forceReload: true,
    });
