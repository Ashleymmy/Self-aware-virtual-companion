...
async function runCommandCaptured(command, commandArgs, options = {}) {
  const captureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'savc-tool-learner-capture-'));
  ...
  try {
    ...
    const result = await runCommand('bash', ['-lc', shellCommand], options);
...
async function prepareOpenClawCliEnv(repoRoot) {
  const runtimeRoot = path.join(repoRoot, '.runtime', 'tool-learner-openclaw');
...
async function runOpenClaw(repoRoot, args) {
  ...
  const env = await prepareOpenClawCliEnv(repoRoot);

  const primary = await runCommandCaptured('bash', [scriptPath, ...args], {
    cwd: repoRoot,
    env,
  });
  if (primary.code === 0) {
  ...
  const fallback = await runCommandCaptured('node', [modulePath, ...args], {
    cwd: repoRoot,
    env,
  });
  if (fallback.code === 0) {
...
async function discoverTools(options) {
  ...
  const workspace = path.resolve(options.workspace || 'packages/core');
