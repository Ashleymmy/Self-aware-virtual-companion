...
async function runDispatcher(params) {
  const args = [
    path.join(params.repoRoot, 'scripts', 'runtime', 'proactive_dispatcher.mjs'),
    'send',
    '--repo',
    params.repoRoot,
    '--workspace',
...
async function executeTick(options) {
  ...
  const workspace = path.resolve(options.workspace || path.join(repoRoot, 'packages/core'));
...
async function main() {
  ...
  if (command === 'send') {
    await runDispatcher({
      ...
      workspace: path.resolve(args.workspace || 'packages/core'),
