...
for file in \
  ...
  "packages/plugin/src/run-store.ts" \
...
if savc_ensure_openclaw_ready >/tmp/phase4b_plugin_real_openclaw_ready.log 2>&1; then
  pass "openclaw readiness passed"
else
  fail "openclaw readiness failed (see /tmp/phase4b_plugin_real_openclaw_ready.log)"
fi

if (cd "${REPO_ROOT}/packages/plugin" && pnpm exec vitest run src/*.test.ts) >/tmp/phase4b_plugin_real_vitest.log 2>&1; then
...
BASE_PORT="${PHASE4B_PLUGIN_REAL_PORT:-18795}"
PORT=""
...
if bash "${REPO_ROOT}/scripts/lifecycle/phase4b_enable_plugin.sh" --config "${TMP_CONFIG}" --spawn-mode real --sync-agents >/tmp/phase4b_enable_real.log 2>&1; then
...
assert isinstance(sessions_send.get("ok"), bool)
send_status = sessions_send.get("status")
assert send_status is None or (isinstance(send_status, str) and send_status)
...
assert value in {"running", "completed", "failed", "timeout", "unknown"}
print("ok")
PY
then
...
if [[ -z "${DISCORD_BOT_TOKEN:-}" || -z "${DISCORD_CHANNEL_ID:-}" ]]; then
  ...
  if OPENCLAW_CONFIG_PATH="${TMP_CONFIG}" OPENCLAW_GATEWAY_TOKEN="${TOKEN}" \
    node "${OPENCLAW_ROOT}/openclaw.mjs" message send --channel discord --target "${DISCORD_CHANNEL_ID}" \
