...
for file in \
  ...
  "packages/core/orchestrator/voice.mjs" \
  "tests/orchestrator/live2d.test.mjs" \
  "tests/orchestrator/live2d-voice-chain.test.mjs" \
  "packages/plugin/src/tool-live2d-signal.ts" \
  "packages/plugin/src/tool-live2d-signal.test.ts" \
  "packages/plugin/src/tool-voice-call.ts" \
  "packages/ui/src/ui/live2d-runtime.ts" \
  "packages/ui/src/ui/live2d-channel.ts" \
  "packages/ui/src/ui/views/chat.ts"; do
...
if (cd "${REPO_ROOT}" && pnpm --dir packages/ui build) >/tmp/phase6_savc_ui_build.log 2>&1; then
  pass "packages/ui build passed"
else
  fail "packages/ui build failed (see /tmp/phase6_savc_ui_build.log)"
fi
...
if (cd "${REPO_ROOT}/packages/plugin" && pnpm exec vitest run src/*.test.ts) >/tmp/phase6_plugin_vitest.log 2>&1; then
  pass "plugin vitest suite passed"
else
  fail "plugin vitest suite failed (see /tmp/phase6_plugin_vitest.log)"
fi

if savc_ensure_openclaw_ready >/tmp/phase6_openclaw_ready.log 2>&1; then
  pass "openclaw readiness passed"
else
  fail "openclaw readiness failed (see /tmp/phase6_openclaw_ready.log)"
fi
...
if bash "${REPO_ROOT}/scripts/lifecycle/phase5_enable_plugins.sh" --config "${TMP_CONFIG}" --spawn-mode mock --sync-agents >/tmp/phase6_enable.log 2>&1; then
