...
require_file "scripts/runtime/memory_semantic.mjs"
require_file "scripts/runtime/memory_runtime.mjs"
require_file "scripts/runtime/proactive_daemon.mjs"
...
node "${REPO_ROOT}/scripts/runtime/memory_runtime.mjs" write \
  --workspace "${WORKSPACE}" \
  --date "2026-02-10" \
  --topic "Phase4a语义检索" \
...
if node "${REPO_ROOT}/scripts/runtime/memory_runtime.mjs" search \
  --workspace "${WORKSPACE}" \
  --query "Python" \
  --mode keyword \
...
node "${REPO_ROOT}/scripts/runtime/memory_runtime.mjs" load \
  --workspace "${WORKSPACE}" \
  --days 2 \
  --query "Python" \
  --max-tokens 2000 \
...
if node "${REPO_ROOT}/scripts/runtime/memory_runtime.mjs" auto-recall \
  --workspace "${WORKSPACE}" \
  --query "Python" \
...
if node "${REPO_ROOT}/scripts/runtime/memory_runtime.mjs" auto-capture \
  --workspace "${WORKSPACE}" \
...
if node "${REPO_ROOT}/scripts/runtime/memory_semantic.mjs" store \
  --workspace "${WORKSPACE}" \
...
if node "${REPO_ROOT}/scripts/runtime/memory_semantic.mjs" search \
  --workspace "${WORKSPACE}" \
...
node "${REPO_ROOT}/scripts/runtime/memory_semantic.mjs" migrate "${WORKSPACE}/memory/semantic" \
...
node "${REPO_ROOT}/scripts/runtime/memory_semantic.mjs" health --workspace "${WORKSPACE}" --json >"${TMP_ROOT}/health.json"
...
if bash "${REPO_ROOT}/scripts/test/test_phase1.sh" >/tmp/phase4a_regress_phase1.log 2>&1; then
  pass "phase1 regression passed"
else
  fail "phase1 regression failed (see /tmp/phase4a_regress_phase1.log)"
fi

if bash "${REPO_ROOT}/scripts/test/test_phase3.sh" >/tmp/phase4a_regress_phase3.log 2>&1; then
