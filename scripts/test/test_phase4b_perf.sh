...
for file in \
  ...
  "packages/core/orchestrator/aggregator.mjs"; do
...
import { discoverAgents, getAgent } from './packages/core/orchestrator/registry.mjs';
import { spawnAgent, waitForAgent, waitForAll } from './packages/core/orchestrator/lifecycle.mjs';
...
await discoverAgents('packages/core/agents', { forceReload: true });
...
for (let i = 0; i < 80; i += 1) {
  ...
  await routeMessage('抱抱我', { agentsDir: 'packages/core/agents' });
  level1Times.push(performance.now() - s1);

  const s2 = performance.now();
  await routeMessage('帮我优化这段 SQL 查询', { agentsDir: 'packages/core/agents' });
