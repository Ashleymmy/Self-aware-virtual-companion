...
# 1) Required files
require_file "packages/core/persona/PERSONA.md"
require_file "packages/core/persona/voice.yaml"
require_file "packages/core/persona/values.yaml"
require_file "packages/core/skills/memory-manager/SKILL.md"
require_file "tests/phase1-dialogue-scenarios.md"
require_file "scripts/runtime/memory_runtime.mjs"
require_file "scripts/test/test_phase1_runtime.sh"
require_contains "scripts/runtime/memory_runtime.mjs" "compress-window"
require_contains "scripts/runtime/memory_runtime.mjs" "search"

for path in \
  "packages/core/memory/episodic/index.md" \
  "packages/core/memory/semantic/user-profile.md" \
  "packages/core/memory/semantic/knowledge-base.md" \
  "packages/core/memory/semantic/facts.md" \
  "packages/core/memory/procedural/workflows.md" \
  "packages/core/memory/procedural/tool-usage.md" \
  "packages/core/memory/emotional/relationship.md" \
  "packages/core/memory/emotional/mood-log.md" \
  "packages/core/memory/emotional/milestones.md" \
  "packages/core/memory/tools/available.md" \
  "packages/core/memory/tools/learning-queue.md" \
  "packages/core/memory/growth/milestones.md"; do
  require_file "${path}"
done
...
for heading in \
  ...
  "# 兴趣爱好" \
  "# 关系定位"; do
  require_contains "packages/core/persona/PERSONA.md" "${heading}"
done
...
for p in ['packages/core/persona/voice.yaml', 'packages/core/persona/values.yaml']:
...
for token in \
  ...
  "on_keyword: \"记得吗\"" \
  "on_keyword: \"你还记得\""; do
  require_contains "packages/core/skills/memory-manager/SKILL.md" "${token}"
done

# 5) Runtime memory validation (write/read/compress)
if bash "${REPO_ROOT}/scripts/test/test_phase1_runtime.sh" >/tmp/phase1_runtime_test.log 2>&1; then
