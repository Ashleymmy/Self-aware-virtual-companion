...
cd "${REPO_ROOT}"
TOOL_RUNTIME="${REPO_ROOT}/scripts/runtime/tool_learner_runtime.mjs"
REFLECT_RUNTIME="${REPO_ROOT}/scripts/runtime/self_reflection_runtime.mjs"
MEMORY_RUNTIME="${REPO_ROOT}/scripts/runtime/memory_runtime.mjs"
TUNING_RUNTIME="${REPO_ROOT}/scripts/runtime/persona_tuning_runtime.mjs"

TMP_ROOT="/tmp/savc_phase3_runtime"
WORKSPACE="${TMP_ROOT}/packages-core"
...
# 1) Required files
require_file "packages/core/skills/tool-learner/SKILL.md"
require_file "packages/core/skills/self-reflection/SKILL.md"
require_file "scripts/runtime/tool_learner_runtime.mjs"
require_file "scripts/runtime/self_reflection_runtime.mjs"
require_file "scripts/runtime/persona_tuning_runtime.mjs"
require_file "scripts/lifecycle/phase3_run_daily.sh"
require_file "scripts/lifecycle/phase3_run_monthly.sh"
require_file "scripts/lifecycle/phase3_cron_install.sh"
require_file "scripts/lifecycle/phase3_cron_remove.sh"
require_file "scripts/test/test_phase3_longrun.sh"

# 2) SKILL checks
require_contains "packages/core/skills/tool-learner/SKILL.md" "name: tool-learner"
require_contains "packages/core/skills/tool-learner/SKILL.md" "on_schedule"
require_contains "packages/core/skills/self-reflection/SKILL.md" "name: self-reflection"
require_contains "packages/core/skills/self-reflection/SKILL.md" "schedule: \"0 23 * * *\""

# 3) Growth monthly summary directory
if [[ -d "${REPO_ROOT}/packages/core/memory/growth/monthly-summary" ]]; then
...
rm -rf "${TMP_ROOT}"
mkdir -p "${WORKSPACE}"
cp -R "${REPO_ROOT}/packages/core/memory" "${WORKSPACE}/memory"
cp -R "${REPO_ROOT}/packages/core/persona" "${WORKSPACE}/persona"
