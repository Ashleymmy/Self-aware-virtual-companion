...
for file in \
  "packages/core/agents/voice.yaml" \
  ...
  "scripts/lifecycle/phase5_enable_plugins.sh"; do
...
if (cd "${REPO_ROOT}/packages/plugin" && pnpm exec vitest run src/*.test.ts) >/tmp/phase5d_plugin_vitest.log 2>&1; then
  pass "plugin vitest suite passed"
else
  fail "plugin vitest suite failed (see /tmp/phase5d_plugin_vitest.log)"
fi

if savc_ensure_openclaw_ready >/tmp/phase5d_openclaw_ready.log 2>&1; then
  pass "openclaw readiness passed"
else
  fail "openclaw readiness failed (see /tmp/phase5d_openclaw_ready.log)"
fi
...
if bash "${REPO_ROOT}/scripts/lifecycle/phase5_enable_plugins.sh" --config "${TMP_CONFIG}" --spawn-mode mock --sync-agents >/tmp/phase5d_enable.log 2>&1; then
...
(
  sleep 1
  savc_local_curl -fsS "http://127.0.0.1:${VOICE_WEBHOOK_PORT}/voice/webhook" \
    -H "content-type: application/json" \
