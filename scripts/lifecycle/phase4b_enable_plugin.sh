...
SPAWN_MODE="mock"
SYNC_AGENTS=1
SAVC_AGENTS_DIR="${REPO_ROOT}/packages/core/agents"
PLUGIN_DIR="${REPO_ROOT}/packages/plugin"

usage() {
  cat <<USAGE
Usage: bash scripts/lifecycle/phase4b_enable_plugin.sh [--config <path>] [--spawn-mode <mock|real>] [--sync-agents] [--dry-run]
...
Options:
  ...
  --sync-agents     Sync agents.list and main.subagents.allowAgents from packages/core/agents (default enabled)
...
python3 - "${CONFIG_PATH}" "${DRY_RUN}" "${SPAWN_MODE}" "${SYNC_AGENTS}" "${SAVC_AGENTS_DIR}" "${PLUGIN_DIR}" <<'PY'
...
plugin_dir = Path(sys.argv[6]).resolve()
...
load = ensure_obj(plugins, "load")
load_paths = load.get("paths")
if not isinstance(load_paths, list):
    load_paths = []
load["paths"] = merge_unique([item for item in load_paths if isinstance(item, str)], [str(plugin_dir)])
