...
usage() {
  cat <<USAGE
Usage: bash scripts/lifecycle/phase5_enable_plugins.sh [--config <path>] [--spawn-mode <mock|real>] [--sync-agents] [--dry-run] [--voice-provider <mock|twilio|telnyx|plivo>]
...
Options:
  ...
  --sync-agents            Sync agents.list and allowAgents from packages/core/agents (default enabled)
...
run_phase4b_patch() {
  ...
  bash "${REPO_ROOT}/scripts/lifecycle/phase4b_enable_plugin.sh" "${args[@]}"
}
