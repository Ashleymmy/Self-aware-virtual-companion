...
async function main() {
  const case1 = await routeMessage('今天好累啊，抱抱我', { agentsDir: 'packages/core/agents' });
  assert.equal(case1.agent, 'companion');
  assert.equal(case1.level, 1);

  const case2 = await routeMessage('帮我优化这段 SQL 查询', { agentsDir: 'packages/core/agents' });
  assert.equal(case2.agent, 'technical');
  assert.equal(case2.level, 2);

  const case3 = await routeMessage('你觉得人生的意义是什么', { agentsDir: 'packages/core/agents' });
